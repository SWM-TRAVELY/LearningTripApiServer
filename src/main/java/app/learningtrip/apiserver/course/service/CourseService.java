...
public class CourseService {
    ...
    public List<CourseThumbnail> getRecommend() throws NoSuchObjectException {
        ...
        for (int i = 0; i < HOME_RECOMMEND_COURSE; i++) {
            ...
            if(!course.get().getName().equals("우리아이 맞춤 여행 코스")) {
                List<CoursePlaceRecommend> coursePlaceRecommendList = coursePlaceRecommendRepository.findAllByCourseIdOrderByDayAscSequenceAsc(
                    Long.valueOf(index));
                List<Place> placeList = new ArrayList<Place>();
                ...
                CourseThumbnail courseThumbnail = CourseThumbnail.toThumbnail(course.get(),
                    placeList);
    ...
    public List<CourseThumbnail> createAutoGeneratedCourse(CourseCreate options){
        List<CourseThumbnail> courseThumbnailList = new ArrayList<CourseThumbnail>();
        CourseThumbnail courseThumbnail = new CourseThumbnail();
//        학년 ID 조회
        int gradeId = gradeRepository.findIdBySchoolGrade(options.getGrade(), options.getGradeOption()).intValue();

//        학년 ID로 해당되는 관광지 필터링
        List<PlaceRelatedInfo> placeList = filterGrade(placeRelatedInfoRepository
            .findAllByProvinceCity(options.getLocation(), options.getLocationOption()), gradeId);

        CourseRecommend courseRecommend = new CourseRecommend();

        courseRecommend.setName("우리아이 맞춤 여행 코스");
        CourseRecommend course = courseRecommendRepository.save(courseRecommend);

//        하루뿐일 때
        if(Objects.equals(options.getStart(), options.getEnd())){
            int dayOfWeek = getDayOfWeek(options.getStart());

            List<Place> placeList1 = getPlaceListByPlaceInfoList(filterDay(placeList, dayOfWeek));

            int max = Math.min(placeList1.size(), 4);

            for(int idx = 1; idx <= max; idx++){
                coursePlaceRecommendRepository.save(new CoursePlaceRecommend(1, idx, course, placeList1.get(idx-1)));
            }

            courseThumbnail.setId((long) course.getId().intValue());
            courseThumbnail.setName("우리아이 맞춤 여행 코스");
            courseThumbnail.setImageURL(placeList1.get(0).getImageURL1());
            courseThumbnail.setPlace1(placeList1.get(0).getName());
            courseThumbnail.setPlace2(placeList1.get(1).getName());
            courseThumbnail.setPlace3(placeList1.get(2).getName());
        ...
        List<Integer> daysList = getDaysList(getDayOfWeek(options.getStart()), getDayOfWeek(options.getEnd()));
        List<List<Place>> placesList = new ArrayList<>();
        List<Long> checkDuplication = new ArrayList<>();

        daysList.forEach(day-> {
            placesList.add(getPlaceListByPlaceInfoList(filterDay(placeList, day)));
        });
        ...
        for(int idx = 0; idx < placesList.size(); idx++){
            ...
            placesList.get(idx).forEach(place -> {
                if(!checkDuplication.contains(place.getId()) && size.get() <= 3){
                    checkDuplication.add(place.getId());
                    coursePlaceRecommendRepository.save(new CoursePlaceRecommend(finalIdx+1,
                        sequence.getAndIncrement(), course, place));
        ...
        courseThumbnail.setId((long) course.getId().intValue());
        courseThumbnail.setName("우리아이 맞춤 여행 코스");
        courseThumbnail.setImageURL(placesList.get(0).get(0).getImageURL1());
        courseThumbnail.setPlace1(placesList.get(0).get(0).getName());
        courseThumbnail.setPlace2(placesList.get(0).get(1).getName());
        courseThumbnail.setPlace3(placesList.get(0).get(2).getName());

        courseThumbnailList.add(courseThumbnail);
        return courseThumbnailList;
    }

    private List<Integer> getDaysList(int start, int end){
        List<Integer> daysList = new ArrayList<>();

        if(start > end){
            for(int idx = start; idx <= end+7; idx++){
                if(idx > 7){
                    daysList.add(idx-7);
                }
                else{
    ...
    private List<PlaceRelatedInfo> filterDay(List<PlaceRelatedInfo> placeList, int day) {
        List<PlaceRelatedInfo> filteredPlaceList = new ArrayList<>();

        placeList.forEach(place -> {
            if (checkBitmask(place.getOpeningDays(), day)) {
                filteredPlaceList.add(place);
            }
        });
    ...
    private List<Place> getPlaceListByPlaceInfoList(List<PlaceRelatedInfo> placeInfoList){
        List<Place> placeList = new ArrayList<>();

        placeInfoList.forEach(place -> {
            placeList.add(placeRepository.findById(place.getId()).orElse(null));
        });
